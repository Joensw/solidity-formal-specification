...
library Lists {
    ...
    /// @notice postcondition forall (uint i) !(0 <= i && i < self.items.length - 1) || (self.items[i] == __verifier_old_bool(self.items[i]))
    ...
    function add(List storage self, bool num) public {
        self.items.push(num);
    }
    ...
    /// @notice postcondition self.items.length == __verifier_old_uint(self.items.length) - 1
    /// @notice postcondition forall (uint i) !(i >= 0 && i < index) || (self.items[i] == __verifier_old_bool(self.items[i]))
    /// @notice postcondition forall (uint i) !(i >= index && i < self.items.length) || (self.items[i] == __verifier_old_bool(self.items[i + 1]))
    /// @notice postcondition ret == __verifier_old_bool(self.items[index])

    function remove(List storage self, uint index) public returns (bool ret) {
        ...
        /// @notice invariant self.items[self.items.length - 1] ==  __verifier_old_bool(self.items[self.items.length - 1])
        /// @notice invariant self.items.length == __verifier_old_uint(self.items.length)
        /// @notice invariant forall (uint k) !(k >= 0 && k < index) || (self.items[k] == __verifier_old_bool(self.items[k]))
        /// @notice invariant forall (uint k) !(k >= index && k < i) || (self.items[k] == __verifier_old_bool(self.items[k + 1]))
        /// @notice invariant forall (uint k) !(k >= i && k < self.items.length) || (self.items[k] == __verifier_old_bool(self.items[k]))
    ...
    function removeObject(List storage self, bool val) public returns (bool success, uint outIndex) { 
        bool found = false;
        uint index = 0;
    ...
    function get(List storage self, uint index) public view returns (bool ret) {
        require(index < self.items.length);
        require(index >= 0);
    ...
    /// @notice postcondition forall (uint i) !(i >= 0 && i < index) || (self.items[i] == __verifier_old_bool(self.items[i]))
    /// @notice postcondition forall (uint i) !(i > index && i < self.items.length) || (self.items[i] == __verifier_old_bool(self.items[i]))
    /// @notice postcondition self.items[index] == newValue

    function set(List storage self, uint index, bool newValue) public {
        require(index < self.items.length);
        require(index >= 0);
