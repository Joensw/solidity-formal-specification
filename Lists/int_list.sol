...
library Lists {
    ...
    /// @notice postcondition forall (uint i) !(0 <= i && i < self.items.length - 1) || (self.items[i] == __verifier_old_int(self.items[i]))
    ...
    function add(List storage self, int num) public {
        self.items.push(num);
    }
    ...
    function contains(List storage self, int num) public view returns (bool isContained) {
    ...
    /// @notice postcondition forall (uint i) !(i >= index && i < self.items.length) || (self.items[i] == __verifier_old_int(self.items[i + 1]))
    /// @notice postcondition ret == __verifier_old_int(self.items[index])

    function remove(List storage self, uint index) public returns (int ret) {
        ...
        /// @notice invariant self.items[self.items.length - 1] ==  __verifier_old_int(self.items[self.items.length - 1])
        /// @notice invariant self.items.length == __verifier_old_uint(self.items.length)
        /// @notice invariant forall (uint k) !(k >= 0 && k < index) || (self.items[k] == __verifier_old_int(self.items[k]))
        /// @notice invariant forall (uint k) !(k >= index && k < i) || (self.items[k] == __verifier_old_int(self.items[k + 1]))
        /// @notice invariant forall (uint k) !(k >= i && k < self.items.length) || (self.items[k] == __verifier_old_int(self.items[k]))
    ...
    /// @notice postcondition exists (uint i) !(success) || (i >= 0 && i < __verifier_old_uint(self.items.length) && __verifier_old_int(self.items[i]) == val) 
    /// @notice postcondition forall (uint i) !(i >= 0 && i < __verifier_old_uint(self.items.length) && __verifier_old_int(self.items[i]) == val) || (success)
    /// @notice postcondition !(success) || self.items.length == __verifier_old_uint(self.items.length) - 1
    /// @notice postcondition forall (uint i) !(success) || !(i >= 0 && i < outIndex) || (self.items[i] == __verifier_old_int(self.items[i]))
    /// @notice postcondition forall (uint i) !(success) || !(i >= outIndex && i < self.items.length) || (self.items[i] == __verifier_old_int(self.items[i + 1]))
    /// @notice postcondition (success) || self.items.length == __verifier_old_uint(self.items.length)

    function removeObject(List storage self, int val) public returns (bool success, uint outIndex) { 
        bool found = false;
        uint index = 0;
    ...
    /// @notice postcondition forall (uint i) !(i >= 0 && i < __verifier_old_uint(self.items.length) && __verifier_old_int(self.items[i]) == val) || (valid)
    /// @notice postcondition exists (uint i) !(valid) || (i >= 0 && i < __verifier_old_uint(self.items.length) && __verifier_old_int(self.items[i]) == val)  
    ...
    function get(List storage self, uint index) public view returns (int ret) {
        require(index < self.items.length);
        require(index >= 0);
